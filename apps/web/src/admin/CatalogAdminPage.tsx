...
export function CatalogAdminPage() {
  ...
  return <main className="page-shell"><p className="eyebrow">Administrative control plane</p><h1>Curriculum studio</h1><p role="status">{message}</p>
    <form className="admin-form" onSubmit={(event) => void createDomain(event)}><h2>1. Ecosystem</h2>
      <label className="form-field">Ecosystem name<input name="name" required placeholder="Example: Java"/></label>
      <label className="form-field">Ecosystem slug<input name="slug" required placeholder="Example: java"/></label>
      <label className="form-field">Description<textarea name="description" required placeholder="Describe this ecosystem"/></label>
      <label className="form-field">Display order<input name="order" type="number" min="1" defaultValue="1"/></label>
      <button>Create ecosystem</button>
    </form>
    ...
    {domain && <form className="admin-form" onSubmit={(event) => void createTechnology(event)}><h2>2. Technology in {domain.name}</h2>
      <label className="form-field">Technology name<input name="name" required placeholder="Example: Core Java"/></label>
      <label className="form-field">Technology slug<input name="slug" required placeholder="Example: core-java"/></label>
      <label className="form-field">Description<textarea name="description" required placeholder="Describe this technology"/></label>
      <button>Create technology</button>
    </form>}
    {technology && <form className="admin-form" onSubmit={(event) => void createTopic(event)}><h2>3. Topic in {technology.name}</h2>
      <label className="form-field">Topic title<input name="title" required placeholder="Example: Collections"/></label>
      <label className="form-field">Topic slug<input name="slug" required placeholder="Example: collections"/></label>
      <label className="form-field">Summary<textarea name="summary" required placeholder="Summarize this topic"/></label>
      <label className="form-field">Learning objective<input name="objective" required placeholder="What should the learner understand?"/></label>
